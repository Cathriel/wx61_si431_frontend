<template>
  <v-app>
    ...
    <v-container fluid class="ma-3">
      <v-row dense>
        <v-col v-for="favouritePost in retrieveFavouritePosts(getCurrentUserId)" :key="favouritePost.id" cols="12" sm="3" xs="1">
          <v-card height="450px">
            <v-img :src="cards[favouritePost.id]" height="200px" style="float: right; margin: 0px 0px 15px 15px;">
              <v-card-actions>
                <v-btn icon color="white" @click="deleteFavourite(getCurrentUserId, favouritePost.id)">
...
export default {
  ...
  methods:{
    ...
    retrieveFavouritePosts(id){
      FavouritePostApiService.getAllByLeaseholderId(id)
          .then(response => {
